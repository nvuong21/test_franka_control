...
void HybridForceController::starting(const ros::Time&) {
  ...
  force_prev_.setZero();
  last_tau_d_ = Eigen::Map<Eigen::Matrix<double, 7, 1>>(initial_state.tau_J_d.data());
...
void HybridForceController::update(const ros::Time&, const ros::Duration& period) {
  ...
  Eigen::VectorXd desired_force_torque(6), force_control(6), force_filter(6),
      force_est(6), tau_force(7), tau_task(7), tau_cmd(7), tau_fric(7);

  Eigen::MatrixXd jacobian_transpose_pinv;
  pseudoInverse(jacobian.transpose(), jacobian_transpose_pinv);
  Eigen::Map<Eigen::Matrix<double, 7, 1> > tau_ext_hat(robot_state.tau_ext_hat_filtered.data());
  force_est = jacobian_transpose_pinv * (tau_ext_hat);
  // force_filter_ = 0.4 * force_est + 0.6 * force_filter_;
  ...
  dq_filter_    = gain * dq + (1- gain) * dq_filter_;

  // Friction torque
  for (size_t i = 0; i < 7; ++i){
    tau_fric(i) = friction_coeffs_(i, 0) / (1 + std::exp(-friction_coeffs_(i, 1) *
                  (dq_filter_(i) + friction_coeffs_(i, 2)))) -
                  friction_coeffs_(i, 0) / (1 + std::exp(friction_coeffs_(i, 1) * friction_coeffs_(i, 2))) ;
  }
  // std::cout << tau_fric << std::endl;
  ...
  double ki{ki_f_};
  ki = ki_f_ / (1 + std::pow(200, std::fabs(desired_force_torque(2) - force_filter_(2)  + force_ext_initial_(2)) - 0.4 * desired_force_));

  double kd{kd_f_};
  kd = kd_f_ / (1 + std::pow(1000, 0.4*desired_force_ - std::fabs(desired_force_torque(2) - force_filter_(2)  + force_ext_initial_(2))));
  ...
  force_control = desired_force_torque + kp_f_ * (desired_force_torque - force_filter_ + force_ext_initial_) +
                   force_error_ - kd * jacobian * dq_filter_;
  ...
  tau_task << jacobian.transpose() *
                  (-stiffness_m_ * error - damping_m_ * (jacobian * dq_filter_ - twist_d_));
  ...
  for (size_t i = 0; i < 7; ++i)
    ...
    // joint_handles_[i].setCommand(tau_fric(i));
  ...
  if (rate_trigger_() && publisher_.trylock()) {
    ...
    // publisher_.msg_.x = force_ext(2);
    publisher_.msg_.y = desired_force_torque(2);
    publisher_.msg_.z = kd;
    ...
    // Compare measured torque and command torque and friction torque
    for (size_t i = 0; i < 7; ++i) {
      publisher_.msg_.data1[i] = last_tau_d_(i);
      publisher_.msg_.data2[i] = tau_J(i) - gravity(i);
      publisher_.msg_.data3[i] = tau_J(i) - gravity(i) + tau_fric(i);
      // publisher_.msg_.data2[i] =
    }
